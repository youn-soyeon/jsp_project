...
public class BoardDAO {
	...
	public int getListCount() {
		...
		try {
			...
			if (rs.next()) {
				x = rs.getInt(1); // 첫번째 컬럼 값을 가져온다 => Count(*)의 값
			}

		} catch (Exception ex) {
			System.out.println("getListCount 에러 : " + ex);

		} finally { // 작업이 끝났으므로 모든객체들 반납(DB access에 사용되었음)
			if (rs != null)
				try {
					rs.close();
	...
	public List getBoardList(int page, int limit) {
		String board_list_sql = "select * from " + "(select rownum rnum, BOARD_NUM, BOARD_ID, BOARD_SUBJECT,"
				...
				+ "BOARD_RE_SEQ,BOARD_READCOUNT, BOARD_DATE from " + "(select * from memberboard order by "
		...
		try {
			...
			while (rs.next()) {
				...
				board.setBOARE_READCOUNT(rs.getInt("BOARD_READCOUNT"));
				board.setBOARD_DATE(rs.getDate("BOARD_DATE"));
				list.add(board);
			}
			return list;
		} catch (Exception ex) {
			System.out.println("getBaordList 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	public int boardReply(BoardBean board) {
		...
		try {
			...
			sql = "update memberboard set BOARD_RE_SEQ=BOARD_RE SEQ+1";
			sql += "where BOARD_RE_REF=? and BOARD_RESEQ>?";
			...
			pstmt.setString(3, board.getBOARD_SUBJECT());
			pstmt.setString(4, board.getBOARD_CONTENT());
			pstmt.setString(5, "");// 답장에는 파일을 업로드 하지않음.
			pstmt.setInt(6, re_ref); // 변한값 세팅
			pstmt.setInt(7, re_lev); // 변한값 세팅
			pstmt.setInt(8, re_seq); // 변한값 세팅
			pstmt.setInt(9, 0);
	...
	public boolean boardDelete(int num) {
		String board_delete_sql = "delect form memberboard where BOARD_num=?";
